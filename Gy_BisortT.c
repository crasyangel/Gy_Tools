...
{  
    ...
    if (-1 == seach_ret) /* 查找不成功，插入节点 */
    {
        ...
        if (!f) 
        {
			GY_PRINT("insert [%d,%d] as root!!!\n", s->data[0], s->data[1]);
            *T = s;            /*  插入s为新的根结点 */
		}
        else if (-1 == Data_Cmp(data, f->data)) 
        {
			GY_PRINT("insert [%d,%d] as left child of [%d,%d]!!!\n",
					s->data[0], s->data[1], f->data[0], f->data[1]);
            f->lchild = s;    /*  插入s为左孩子 */
		}
        else if (1 == Data_Cmp(data, f->data)) 
        {
			GY_PRINT("insert [%d,%d] as right child of [%d,%d]!!!\n",
					s->data[0], s->data[1], f->data[0], f->data[1]);
            f->rchild = s;  /*  插入s为右孩子 */
        }
    } 
    else if(1 == seach_ret)
    {
        ...
        if (f) 
        {
			GY_PRINT("need to delete [%d,%d] before insert [%d,%d]\n", 
		...
		{
			GY_PRINT("insert [%d,%d] error!!!\n", data[0], data[1]);
		}
    }
	else if(0 == seach_ret)
    {
    	/*  树中已有关键字相同的结点，不再插入 */
       	GY_PRINT("no need to insert [%d,%d], already have [%d,%d]!!!\n"
...
bool CreateQamNameBiTree(BiTree *T, CHAR_T *rangebuf)
{
	...
	if(!rangebuf)
	{
		GY_PRINT("you transfer a null buf!!\n");
		return false;
	}

	p = strstr(rangebuf, "[["); //区间开始
	while(p)
	{
	    q = p+strlen("[[");
		token = strtok_str(q, "],[");
		...
		while(token != NULL)
		{
			...
			if(strstr(token, "]]"))
			{
				GY_PRINT("reach the end descriptor!!\n");
				break;
			}
	...
	GY_PRINT("saved %d qam_name region!!!\n", qam_num);
